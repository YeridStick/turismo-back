...
public class BrevoEmailAdapter implements EmailGateway {
    ...
    public Mono<Void> sendEmail(EmailMessage message) {
        BrevoEmailRequest body = new BrevoEmailRequest(
                new Sender(senderName, message.to()), // Temporal: remitente = destino para evitar bloqueos
                List.of(new Recipient(message.to())),
                message.subject(),
                message.htmlBody()
        );

        LOG.info("Brevo requestBody sent to={}", message.to());
                ...
                .exchangeToMono(response -> response.bodyToMono(String.class)
                        ...
                        .flatMap(payload -> {
                            if (response.statusCode().isError()) {
                                ...
                                return Mono.error(new IllegalStateException("Brevo send failed: " + response.statusCode().value() + " - " + payload));
                            }
                            LOG.info("Brevo sendEmail success status={} response={}", response.statusCode().value(), payload);
                            return Mono.empty();
                        }))
                .doOnError(e -> LOG.error("Brevo connectivity error", e))
                .then();
    }
